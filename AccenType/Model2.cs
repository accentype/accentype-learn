...
{
    public class Model2 : ILanguageModel
    {
        ...
        private int[] m_HashTableByteIndex;

        private byte[] m_PredictionAsciiBytes;
        private int[] m_PredictionAsciiIndex;
        private int[] m_PredictionCounts;

        private int m_NumAccString;
        ...
        private AccentConverter m_Converter = new VietConverter();
        ...
        public Dictionary<string, int> GetAccents(string rawSegment)
        {
            ...
            int hashCode = (int)m_HashFunction.Search(Encoding.UTF8.GetBytes(rawSegment));
            if (hashCode >= 0 && hashCode < m_HashFunction.N)
            {
                int hashBegin = m_HashTableByteIndex[hashCode];
                int hashEnd = m_HashTableByteIndex[hashCode + 1];

                byte[] asciiBytes = hashEnd > hashBegin ? new byte[hashEnd - hashBegin] : null;
                for (int i = hashBegin; i < hashEnd; i++)
                {
                    asciiBytes[i - hashBegin] = m_HashTableBytes[i];
                }
                ...
                if (asciiBytes != null)
                {
                    ...
                    for (int iElement = 0; iElement < count; iElement++)
                    {
                        ...
                        int begin = m_PredictionAsciiIndex[accCodeIndex];
                        int end = m_PredictionAsciiIndex[accCodeIndex + 1];
                        
                        var accStringBytes = new byte[end - begin];
                        for (int i = begin; i < end; i++)
                        {
                            accStringBytes[i - begin] = m_PredictionAsciiBytes[i];
                        }

                        string accString = Encoding.ASCII.GetString(accStringBytes);
